...
```{r}
plot(g, edge.arrow.size=0.2, vertex.size=7, vertex.label.cex=0.7)
```

## Changing the layout-- circle

```{r}
plot(g, edge.arrow.size=0.2, vertex.size=7, vertex.label.cex=0.7, layout=layout_in_circle)
```

## Changing the layout -- sphere
```{r}
plot(g, edge.arrow.size=0.2, vertex.size=7, vertex.label.cex=0.7, layout=layout_on_sphere)
```
...
plot(g, edge.arrow.size=0.2, vertex.size=7, vertex.label.cex=0.7, layout=l)
```
...
## Curved edges
```{r}
plot(g, edge.arrow.size=0.2, vertex.size=7, vertex.label.cex=0.7, layout=l, edge.curved=TRUE)
```
...
g <- graph.data.frame(storm)
ggnet2(g, label=TRUE, edge.size = "ScaledWeight", node.size = 4)
```
...
Looks like what Andre Beveridge and Jie Shan got!
...
storm_ordered <- storm_ordered %>%
  mutate(Source = fct_reorder(Source, ec),
         Target = fct_reorder(Target, ec))
